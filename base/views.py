...
# rooms = [
#     {'id' : 1, 'name' : 'Let\'s play something!'},
#     {'id' : 2, 'name' : 'Sing with me!'},
#     {'id' : 3, 'name' : 'Just Chilling'},
# ]
...
def room(request, pk):
    ...
    room_messages = room.messages_set.all() # messages_set - gives all the children of a specific object from a model (in this case, model = Messages)
...
def updateRoom(request, pk):
    room = Room.objects.get(id=pk)
    form = RoomForm(instance=room) # instance - prefills the form with the values of the passed room
    topics = Topic.objects.all()
...
def activityPage(request):
    room_messages = Messages.objects.all()[0:5]
